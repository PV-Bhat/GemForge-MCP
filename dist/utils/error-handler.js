...
export class GeminiError extends Error {
     ...
     */
    constructor(message, status, code = 'UNKNOWN_ERROR', isRateLimit = false, isRetriable = false) {
        super(message);
 ...
 */
export function handleApiError(error, modelId) {
    ...
    if (error instanceof Error) {
        ...
        if (message.includes('invalid') ||
            ...
            return new GeminiError(`Invalid request to ${modelDisplay}: ${message}`, 400, 'INVALID_REQUEST', false, false);
        }
        ...
        if (message.includes('auth') ||
            ...
            return new GeminiError(`Authentication error with ${modelDisplay}: ${message}`, 401, 'UNAUTHORIZED', false, false);
        }
        // Permission issues
        if (message.includes('permission') ||
            ...
            return new GeminiError(`Permission error for ${modelDisplay}: ${message}`, 403, 'FORBIDDEN', false, false);
        }
        // Not found issues
        if (message.includes('not found') ||
            ...
            return new GeminiError(`Model not found: ${modelDisplay}. Please check the model name and try again.`, 404, 'NOT_FOUND', false, false);
        }
        ...
        if (message.includes('rate') ||
            ...
            return new GeminiError(`Rate limit exceeded for ${modelDisplay}. Please reduce your request frequency and try again later.`, 429, 'RATE_LIMIT_EXCEEDED', true, true);
        }
        ...
        if (message.includes('safety') ||
            ...
            return new GeminiError(`Content filtered by ${modelDisplay} safety systems: ${message}`, 400, 'SAFETY_FILTER', false, false);
        }
        // Server-side issues
        if (message.includes('server') ||
            ...
            return new GeminiError(`${modelDisplay} server error: ${message}. Please try again later.`, 500, 'SERVER_ERROR', false, true);
        }
        ...
        if (message.includes('file') ||
            ...
            return new GeminiError(`File processing error with ${modelDisplay}: ${message}`, 400, 'FILE_ERROR', false, false);
        }
    }
    ...
    if (error instanceof Error) {
        return new GeminiError(error.message, undefined, 'UNKNOWN_ERROR', error.message.includes('rate limit'), error.message.includes('rate limit'));
    }
    // Handle completely unknown errors
    return new GeminiError(`Unknown error occurred with ${modelDisplay}`, undefined, 'UNKNOWN_ERROR', false, false);
}
/**
