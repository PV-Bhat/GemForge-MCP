 ...
 */
export function buildRequest(internal) {
...
export function buildSearchRequest(options) {
    const { query, modelId = MODELS.FLASH, toolName } = options;
    // Build system prompt
 ...
 * @returns {SdkRequest} - Gemini API request body
 */
export function buildReasoningRequest(options) {
    const { problem, modelId = MODELS.FLASH_THINKING, showSteps = true, toolName } = options;
    // Build system prompt
 ...
 * @returns {SdkRequest} - Gemini API request body
 */
export function buildProcessingRequest(options) {
    const { content = '', filePath, operation = 'analyze', modelId = MODELS.FLASH_LITE, toolName } = options;
...
**Model Used:** ${model_id || "Default model"}`;
    ...
    if (codebase_path) {
        ...
        try {
            ...
            const consolidatedText = `Based *only* on the following XML codebase context, answer this question: ${question}\n\n` +
            ...
            throw new McpError(ErrorCode.InvalidParams, `Cannot read codebase file: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
 ...
 * @returns {SdkRequest} - Gemini API request body
 */
export function buildFileopsRequest(args) {
    const { file_path, instruction, operation, model_id } = args;
    if (!file_path) {
 ...
 */
function buildOperationPrompt(operation, content, isImageFile = false) {
