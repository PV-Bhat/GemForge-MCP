...
// Import handlers
import { handleSearch, handleReason } from './handlers/unified-gemini.js';
import { handleDirectSearch } from './handlers/direct-search.js';
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      try {
        ...
        if (request.params.name === 'prompts/list') {
          ...
          throw new McpError(
            ErrorCode.MethodNotFound,
            `Method 'prompts/list' is not supported by this server. Please use one of the available tools listed in the tools endpoint.`
          );
        }
