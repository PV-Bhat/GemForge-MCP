...
import { SearchArgs, ReasonArgs } from '../interfaces/tool-args.js';
 ...
 */
export async function handleProcess(request: any) {
  try {
    if (!request.params.arguments) {
      throw new McpError(
        ErrorCode.InvalidParams,
        'Arguments are required for process handler'
      );
    }
...
export async function handleAnalyze(request: any) {
  try {
    if (!request.params.arguments || !request.params.arguments.file_path) {
      throw new McpError(
        ErrorCode.InvalidParams,
        'file_path parameter is required for analyze handler'
      );
    }
...
export async function handleReason(request: any) {
  try {
    ...
    const args = request.params.arguments as ReasonArgs;
