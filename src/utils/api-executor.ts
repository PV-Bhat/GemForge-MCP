 ...
 */
export async function executeRequest(modelId: string, sdkPayload: any, isRetry: boolean = false) {
  try {
    ...
    console.error('[executeRequest] Debug info: ' + JSON.stringify({
      ...
      roles: req.contents.map((c: any) => c.role)
    }));
    ...
    console.error('[executeRequest] Request details: ' + JSON.stringify({
      ...
      systemInstruction: !!req.systemInstruction,
      roles: req.contents.map((c: any) => c.role),
      partCounts: req.contents.map((c: any) => c.parts.length),
      partTypes: req.contents.map((c: any) => c.parts.map((p: any) => Object.keys(p).join('/')))
    }));
    ...
    console.error('[executeRequest] FINAL SDK PAYLOAD: ' + JSON.stringify({
      contents: req.contents.map((c: any) => ({
        ...
        partTypes: c.parts.map((p: any) => Object.keys(p).join('/'))
      })),
    ...
    console.error('[PAYLOAD] ' + JSON.stringify({
      ...
      hasInlineData: req.contents?.[0]?.parts.some((p: any) => p.inlineData),
      hasFileData: req.contents?.[0]?.parts.some((p: any) => p.fileData),
      partTypes: req.contents?.[0]?.parts.map((p: any) => Object.keys(p).join('/'))
    }));
    ...
    if (req.file_context || req.file_path) {
      ...
      const hasFilePartsInContents = req.contents.some((c: any) => c.parts.some((p: any) => p.inlineData || p.fileData));
      ...
      if (forceProcessing || !hasFilePartsInContents) {
        ...
        const processedRequest = await transformToSdkFormat(tempRequest as any, modelId);
        ...
        if (processedRequest.contents &&
          ...
          if (fileParts.length > 0) {
            ...
            const userMsgIndex = req.contents.findIndex((c: any) => c.role === 'user');
            if (userMsgIndex >= 0) {
    ...
    const result = await model.generateContent({
      ...
      tools: req.toolName === 'gemini_search'
             ? [{ googleSearch: {} } as any]        // correct Node.js SDK shape
             : undefined,
      generationConfig: req.generationConfig,
      safetySettings: req.safetySettings
    });
    ...
    if (error instanceof Error) {
      ...
      if (errorMessage.includes('not found') || errorCode === 404) {
        throw new McpError(
          ErrorCode.InvalidParams,
          `Model ${modelId} not found or not supported. Please try a different model.`
        );
      }
      // Permission error
      if (errorMessage.includes('permission') || errorCode === 403) {
        throw new McpError(
          ErrorCode.InvalidParams,
          `Permission denied for ${modelId}. Check your API key and permissions.`
        );
      }
      // Bad request error
      if (
