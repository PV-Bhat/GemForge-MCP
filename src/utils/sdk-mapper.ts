 ...
 */
export async function transformToSdkFormat(internalRequest: GeminiRequest, modelId?: string): Promise<GenerateContentRequest> {
...
Please access and process this URL.`
  ...
  console.error(
    `[transformToSdkFormat] parts=${parts.length}, ` +
  ...
  if (internalRequest.contents && Array.isArray(internalRequest.contents)) {
    // Find system message
    const systemMsg = internalRequest.contents.find(content =>
      content && typeof content === 'object' && 'role' in content && content.role === 'system'
    );
    ...
    if (systemMsg && systemMsg.parts && Array.isArray(systemMsg.parts) && systemMsg.parts.length > 0) {
      ...
      if (firstPart && typeof firstPart === 'object' && 'text' in firstPart) {
        systemInstruction = firstPart.text as string;
    ...
    if (isFlash) {
      userContents = internalRequest.contents.filter(content =>
        !(content && typeof content === 'object' && 'role' in content && content.role === 'system')
      );
  ...
  if (isFlash && systemInstruction) {
    ...
    (sdkRequest as any).systemInstruction = systemInstruction;
  }

  // Also check if systemInstruction is directly present in the request
  if ((internalRequest as any).systemInstruction) {
    console.error(`[transformToSdkFormat] Found systemInstruction in request, adding to SDK request`);
    (sdkRequest as any).systemInstruction = (internalRequest as any).systemInstruction;
  }
  ...
  try {
    ...
    if (
      internalRequest.toolName === 'gemini_search' &&
...
export function transformFromSdkResponse(sdkResponse: any): GeminiResponse {
  try {
    ...
    let boundingBoxes: any[] | undefined = undefined;
    ...
    if (boundingBoxes && boundingBoxes.length > 0) {
      ...
      if (internalResponse.candidates && internalResponse.candidates.length > 0 && internalResponse.candidates[0].content) {
        ...
        (internalResponse.candidates[0].content as any).boundingBoxes = boundingBoxes;
      }
    }
